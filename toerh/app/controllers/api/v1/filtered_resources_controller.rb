  ...
  module V1
    class FilteredResourcesController < ApiController
      ...
      def get_resources_by_type
        # return resources by resource_type_id if param value is a number less than total amount of types
        if params[:type].to_i <= Tag.all.count && params[:type].to_i != 0
          return ResourceType.find(params[:type]).resources
        # return resources by type_name
        elsif !params[:type].match(/^[0-9]+$/i)
          ResourceType.where("lower(type_name) = ?", params[:type].downcase).find_each do |type|
